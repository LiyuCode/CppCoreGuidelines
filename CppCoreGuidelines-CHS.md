...
### <a name="Rp-direct"></a>P.1: 用代码直接表达思想

##### 原因

编译器不会阅读代码注释（或设计文档），当然，嗯，许多程序猿也不会。
所编写的代码本身便带有某些语义，特别的，它还能让编译器和其他工具来检查。
...
`month`的第一种声明方式显示地表明要返回一个`Month`，且该`Data`型对象的状态不可改变。
而第二种方式则会让读者感到疑惑，并更有可能导致某些莫名bugs。
...
通过调用设计良好的库函数(要做什么，而非如何做)来表达自己的意图，远比直接用编程语言的特性来重新编写好。
...
C++ 程序猿应该自带标准库的基础知识，并合理使用标准库。
所有的程序猿都应知道正在进行的项目的基础库，并合理使用它
所有的使用这些指南的程序猿都应该知道 [guideline support library](#S-gsl) ， 并合理使用
...
一种更好的方式是明确上述double的意思（指新的速度还是在原来速度上的增量），这会用到单位:
